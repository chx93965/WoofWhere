...
// Fix default icon path issue with Leaflet in many bundlers (optional, for dev warning)
...
export default function Map() {
  ...
  // Function to geocode a location string to coordinates using Nominatim (OpenStreetMap)
  async function geocodeLocation(location: string): Promise<[number, number] | null> {
    try {
  ...
  useEffect(() => {
    ...
    playdates.forEach((p) => {
      if (p.coordinates) {
        const marker = L.marker([p.coordinates[1], p.coordinates[0]], { icon: pawIcon }).addTo(layerGroup);
        marker.bindPopup(
          `<h3 style="font-weight:600;">${p.title}</h3>
          <p>${p.location}</p>
          <p>${p.hostName}</p>
