...
export default function Dashboard() {
  ...
  const handleAddPet = async (partyId, petId) => {
    setSelectedPet(petId);
    setRegistered(false);
    try {
      // Avoid duplicate pet registration
      const response = await partyApi.getById(partyId, {"includePets": true});
      const ifExists = response.data.pets.some(p => p.id === petId);
      if (ifExists) {
        setRegistered(true);
        throw new Error("Pet already registered");
      }

      await partyApi.addPet(partyId, petId);
    } catch (error) {
            ...
            {selectedPlaydate && (
              <div className="space-y-3">
                ...
                <Select
                    ...
                    onValueChange={(value) => handleAddPet(selectedPlaydate.id, value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                ...
                onClick={() => {
                  ...
                  if (registered) {
                    toast({
                      ...
                      description: "This pet have already registered for this playdate.",
                      variant: "destructive",
                    });
                    return;
                  }
